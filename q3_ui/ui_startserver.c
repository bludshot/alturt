...
static int gametype_remap[] = {GT_FFA, GT_TEAM, GT_ASN, GT_CTF};
...
static int GametypeBits( char *string ) {
	...
	while( 1 ) {
		...
		if( Q_stricmp( token, "teamDM" ) == 0 ) {
			bits |= 1 << GT_TEAM;
			continue;
		}
		if( Q_stricmp( token, "teamSurvivor" ) == 0 ) {
			bits |= 1 << GT_TEAMSV;
			continue;
		}
		if( Q_stricmp( token, "asassins" ) == 0 ) {
			bits |= 1 << GT_ASN;
			continue;
		}
		if( Q_stricmp( token, "capandhold" ) == 0 ) {
			bits |= 1 << GT_CAH;
			continue;
		}
