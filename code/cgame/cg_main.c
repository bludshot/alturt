...
static cvarTable_t cvarTable[] = {
	...
	{ &cg_synchronousClients, "g_synchronousClients", "0", 0 },	// communicated by systeminfo
        	{ &cg_enableBreath, "cg_sfxBreathing", "1", CVAR_USERINFO | CVAR_ARCHIVE},
#ifdef MISSIONPACK
...
static void CG_RegisterSounds( void ) {
	...
	if ( cgs.gametype >= GT_TEAM || cg_buildScript.integer ) {
		...
		if ( cgs.gametype == GT_BOMB || cgs.gametype == GT_CTF || cg_buildScript.integer ) {
	...
	cgs.media.respawnSound = trap_S_RegisterSound( "sound/items/respawn1.wav", qfalse );
                  cgs.media.stompSound = trap_S_RegisterSound( "sound/stomp.wav", qfalse );
...
void CG_BuildSpectatorString(void) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		if (cgs.clientinfo[i].infoValid && (cgs.clientinfo[i].team == TEAM_SPECTATOR 
						||cgs.clientinfo[i].team == TEAM_RED_SPECTATOR
						||cgs.clientinfo[i].team == TEAM_BLUE_SPECTATOR  )) {
