...
void PM_StepSlideMove( qboolean gravity ) {
	...
	down[2] -= SMALLSTEPSIZE;
	pm->trace (&trace, start_o,pm->mins, pm->maxs, down, pm->ps->clientNum, MASK_SOLID);
	VectorSet(up, 0, 0, 1);
	...
	// test the player position if they were a stepheight higher
                  //start of the trace is player origin, end of the trace is up STEPSIZE from player origin (18)
	pm->trace (&trace, start_o,pm->mins, pm->maxs, up, pm->ps->clientNum, pm->tracemask);

                
	if ( trace.allsolid ) { //if the entire trace is in a solid we cant step up (step is too high)
		if ( pm->debugLevel ) {
