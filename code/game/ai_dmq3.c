...
int Bot1FCTFCarryingFlag(bot_state_t *bs) {
	if (gametype != GT_BOMB) return qfalse;
...
void BotSetTeamStatus(bot_state_t *bs) {
	...
	switch(bs->ltgtype) {
		...
		case LTG_TEAMACCOMPANY:
			...
			if ( ( (gametype == GT_CTF || gametype == GT_BOMB) && EntityCarriesFlag(&entinfo))
				|| ( gametype == GT_BOMB && EntityCarriesCubes(&entinfo)) ) {
				...
				if (BotTeamLeader(bs)) {
					...
					if ( bs->ltgtype != LTG_GETFLAG &&
						...
						if ((gametype != GT_CTF || (bs->redflagstatus == 0 && bs->blueflagstatus == 0)) &&
							(gametype != GT_BOMB || bs->neutralflagstatus == 0) ) {
							// tell the leader we want to be on offence
							BotVoiceChat(bs, leader, VOICECHAT_WANTONOFFENSE);
							//BotAI_BotInitialChat(bs, "wantoffence", NULL);
							//trap_BotEnterChat(bs->cs, leader, CHAT_TELL);
...
//gonna need to change all this to reflect alturt items  --Xamis
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	if (state->eType != ET_MISSILE || state->weapon != WP_HK69)
		return;
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (bs->setupcount > 0) {
		...
		trap_SetUserinfo(bs->client, userinfo);
		//set the team
		if ( !bs->map_restart && g_gametype.integer != GT_ASN ) {
