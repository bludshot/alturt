...
void CL_AdjustAngles( void ) {
	...
	if ( in_buttons[8].active ) {
		speed = 2.001 * cls.frametime * cl_anglespeedkey->value;
	}else 	if ( in_speed.active ) {
...
void CL_JoystickMove( usercmd_t *cmd ) {
	...
	if ( in_buttons[12].active ^ cl_run->integer ) {
		movespeed = 167;
		cmd->buttons &= ~BUTTON_WALKING;
	} else if ( in_speed.active ^ cl_run->integer ) {
		movespeed = 2;
	} else {
		movespeed = 1;
		cmd->buttons |= BUTTON_WALKING;
