...
static void UI_SetCapFragLimits(qboolean uiVars) {
	int cap = 5;
	int frag = 10;
	if (uiInfo.gameTypes[ui_gameType.integer].gtEnum == GT_BOMB) {
		cap = 4;
	} else if (uiInfo.gameTypes[ui_gameType.integer].gtEnum == GT_BOMB) {
		cap = 15;
	}
	if (uiVars) {
...
static void UI_DrawPlayerModel(rectDef_t *rect) {
  ...
  if (updateModel) {
    ...
    UI_PlayerInfo_SetInfo( &info, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin, WP_M4, qfalse );
...
static void UI_DrawOpponent(rectDef_t *rect) {
  ...
  char team[256];
    ...
    UI_PlayerInfo_SetInfo( &info2, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin, WP_M4, qfalse );
...
static qboolean UI_GameType_HandleKey(int flags, float *special, int key, qboolean resetMap) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
		...
		if (uiInfo.gameTypes[ui_gameType.integer].gtEnum == GT_ASN) {
...
static cvarTable_t		cvarTable[] = {
	...
	{ &ui_teamName, "ui_teamName", "SWAT", CVAR_ARCHIVE },
	{ &ui_opponentName, "ui_opponentName", "Red Dragons", CVAR_ARCHIVE },
	{ &ui_redteam, "ui_redteam", "Red Dragons", CVAR_ARCHIVE },
	{ &ui_blueteam, "ui_blueteam", "SWAT", CVAR_ARCHIVE },
 ...
 { &ui_findPlayer, "ui_findPlayer", "Orion", CVAR_ARCHIVE},
